...
template <typename T> class ShapePerformanceTest : public ::testing::Test {};
...
TYPED_TEST(ShapePerformanceTest, PyramidAndConePerformance)
{
    ...
    std::unique_ptr<Shape3D<TypeParam>> pyramid =
        std::make_unique<Pyramid<TypeParam>>(param);
    std::unique_ptr<Shape3D<TypeParam>> cone =
        std::make_unique<Cone<TypeParam>>(param);
    ...
    EXPECT_LT(elapsed.count(), 1000)
        << "Test wydajnosci nie przeszedl: " << elapsed.count() << "us";
}
